...
def generate_center_one_mask(shape):
    assert len(shape) == 2
    assert shape[1] % 2 == 0
    center = shape[0] // 2
    center_size = shape[0] // 2
...
def generate_center_zero_mask(shape):
    assert len(shape) == 2
    assert shape[1] % 2 == 0
    center = shape[0] // 2
    center_size = shape[0] // 2
...
def generate_full_mask(shape):
    ...
    # ! full mask corresponds to  generation
    assert len(shape) == 2
    assert shape[1] % 2 == 0
...
mask_generators = {
    ...
    "expand": generate_center_one_mask,
    "square": generate_center_zero_mask,
