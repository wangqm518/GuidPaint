...
do
  ...
  do
      ...
      cmd="python two_stage_inpainting.py --config_file configs/config.yaml \
        ...
        --mask_type ${mask_type} \
        --output results/${mask_type}/guidpaint-skip \
        --ddim.schedule_params.schedule_type respace \
        --ddim.schedule_params.infer_step_repace 50,50,25,25,5 \
        --optimize_xt.num_iteration_inp 1 \
        --optimize_xt.num_iteration_guid 1 \
